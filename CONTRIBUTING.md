...
Runtime files are plain Lua plus per-language SQLite packs and the bundled
English G2P model. Build tools require Python 3.10 or newer and Git; they use
...
metadata and final SHA-256, the final G2P model SHA-256, required licenses, and
...
WikiPron, checks out that exact release tag, updates CMUdict, rebuilds only
`data/en/pronunciations.sqlite3`, synchronizes `PLUGIN_VERSION`, and refreshes
...
`data/en/pronunciations.sqlite3`, and `data/en/pack.tsv`, and publishes the
...
language has no broad file, available narrow IPA is used. CMUdict and the
project supplement are added only to English.
...
Outputs use `data/<base-code>/pronunciations.sqlite3` plus a small `pack.tsv`
sidecar used for lazy runtime discovery. Locale variants and WikiPron dialect
...
its release/tag and commit provenance. English additionally requires CMUdict:
...
Optional non-English packs provide IPA only. The English readable converter and
bundled G2P model deliberately remain English-specific.
...
[`data/mfa_english_g2p.SOURCE.txt`](data/mfa_english_g2p.SOURCE.txt). This
